...
class PatchPayment(models.Model):
    ...
    sales_agent = fields.Many2one('sales.agent', string='Sales Agent')
    ...
    def ConfirmPost(self):
        for line in self.patch_payment_line:
            ...
            paymet_send = line.lines_pay_and_reconcile(date,sales_agent)
        self.write({
            'state': 'confirmed'
            })
...
class PatchPaymentLine(models.Model):
    ...
    sales_agent = fields.Many2one('sales.agent', string='Sales Agent', required=True)
    ...
    @api.multi
    def _prepare_lines_payment_vals(self, date ,sales_agent):
        self.ensure_one()
        return {
            'state': 'draft',
            'amount': self.amount,
    ...
    @api.multi
    def lines_pay_and_reconcile(self,date,sales_agent):
        newdate = date
        newsales_agent = sales_agent
