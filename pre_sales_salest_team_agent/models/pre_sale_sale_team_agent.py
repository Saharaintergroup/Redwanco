...
class SaleOrderAgentSalesTeam(models.Model):
    ...
    def onchange_partner__sales_ids(self):
        ...
        if self.partner_id:
            ...
            sales_team = self.env['sale.order.type'].search(['sales_team_id','=',self.type_id.sales_team_id])
            sales_team_order_agents = self.env['sale.order'].search([('agent_id_sales','in',sales_team.agent_ids.ids)])
            # agent_list = []
