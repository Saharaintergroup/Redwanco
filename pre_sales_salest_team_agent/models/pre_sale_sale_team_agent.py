...
class SaleOrderAgentSalesTeam(models.Model):
    ...
    agent_id_sales = fields.Many2one('crm.team', string='Sales Agent')
    ...
    def onchange_partner__sales_ids(self):
        ...
        if self.partner_id:
            ...
            sales_team_agents = self.env['crm.team'].search([('agent_id_sales','in',self.type_id.sales_team_id.agent_ids.ids)])

            domain = {'agent_id_sales': [('id', 'in', sales_team_agents.ids)]}
